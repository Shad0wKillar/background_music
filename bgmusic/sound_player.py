...
from bgmusic.sound_latency import format_latency_probe, make_latency_probe
from bgmusic.sound_mix import DEFAULT_OUTPUT_CEILING, finalize_keyboard_mix
...
class KeyboardSoundPlayer:
    ...
    def play(
        ...
        with self.lock:
            ...
            if len(self.active) >= self.max_polyphony:
                overflow = len(self.active) - self.max_polyphony + 1
                del self.active[:overflow]
    ...
    def _new_probe(
        ...
        probe, self.deep_key_count = make_latency_probe(
            self.logger, key, event_value, receipt_ns, kernel_ms,
            self.deep_key_count, self.deep_key_limit,
        )
        return probe
